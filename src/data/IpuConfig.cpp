...
void IpuConfig::readFileValues(String strFilePath) {
    ...
    // CONFIG 섹션의 값 읽기
	defaultSlot.init(pIniManager->Read("CONFIG", "DEFAULT_SLOT"));
	lastSlot.init(pIniManager->Read("CONFIG", "LASTSLOT"));
	TDateTime dtlastSet = pIniManager->Read("CONFIG", "LASTSET", INIFileManager::DataType::DateTime);
	lastSet.init(dtlastSet);

	// SLOT_A 섹션의 값 읽기
	interfaceIn.init(pIniManager->Read("SLOT_A", "INTERFACE_IN"));
	ipIn.init(pIniManager->Read("SLOT_A", "IP_IN"));
	interfaceOut.init(pIniManager->Read("SLOT_A", "INTERFACE_OUT"));
	ipOut.init(pIniManager->Read("SLOT_A", "IP_OUT"));
	interfaceCam1.init(pIniManager->Read("SLOT_A", "INTERFACE_CAM1"));
	ipCam1.init(pIniManager->Read("SLOT_A", "IP_CAM1"));
