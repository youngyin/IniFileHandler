...
void __fastcall TFormMain::btnSaveClick(TObject* Sender)
{
    ...
    UnicodeString strFilePath =
        TPath::Combine(strFolderPath, "sample.ini"); //파일 이름 및 경로 생성
    ...
    try {
        ...
        pIniFile->WriteDate(
            strSection, "testDate", TDateTime::CurrentDateTime());
        pIniFile->WriteDateTime(
            strSection, "testDateTime", TDateTime::CurrentDateTime());
        pIniFile->WriteTime(
            strSection, "testTime", TDateTime::CurrentDateTime());
        pIniFile->WriteFloat(
            strSection, "testFloat", 2.13111999999999999999999); //반올림처리됨.
...
void __fastcall TFormMain::BtnReadClick(TObject* Sender)
{
    ...
    try {
        ...
        // 인코딩을 UTF-8로 설정하여 파일을 읽습니다.
        FileContent->LoadFromFile(EdtFilePath->Text, TEncoding::UTF8);
